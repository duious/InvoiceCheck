...
public class UtilRedis {
    public static final String BASE_KEY = "invoice:check:datastore:";

    public static final int exTime_short = 300;
    public static final int exTime_long = 600;
    ...
    private static final int EXPIRE = 60000;    //默认缓存时间
    @Autowired
    private static UtilRedis instance;
    private static ReentrantLock lock = new ReentrantLock();

    private UtilRedis() {
    }

    public static UtilRedis getInstance() {
        if (instance == null) {
            lock.lock();
            if (instance == null) {
                instance = new UtilRedis();
            }
            lock.unlock();
        }
        return instance;
...
//
////        String werwe = UtilRedis.getInstance().Strings().set("werwe", JSON.toJSONString(map));
////        System.out.println(werwe);
////        String s = UtilRedis.getInstance().strings().get("werwe");
