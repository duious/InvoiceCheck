...
import com.sjxd.invoicecheckserver.util.Res;
...
public class TokenType {
    ...
    private static String[] pcHeaders = new String[]{"Windows 98", "Windows ME", "Windows 2000", "Windows XP",
            "Windows", "windows", "Windows NT", "Ubuntu"};
    /**
     * 手机浏览器的User-Agent里的关键词
     */
    private static String[] mobileUserAgents = new String[]{"Nokia", // 诺基亚，有山寨机也写这个的，总还算是手机，Mozilla/5.0
            // (Nokia5800 XpressMusic)UC AppleWebkit(like Gecko) Safari/530
            "SAMSUNG", // 三星手机
            // SAMSUNG-GT-B7722/1.0+SHP/VPP/R5+Dolfin/1.5+Nextreaming+SMM-MMS/1.2.0+profile/MIDP-2.1+configuration
            // /CLDC-1.1
    ...
    private static boolean isIOSDevice(HttpServletRequest request) {
        ...
        String userAgent = request.getHeader("user-agent");
        for (int i = 0; userAgent != null && !"".equals(userAgent.trim()) && i < iosSys.length; i++) {
    ...
    private static boolean isWeChat(HttpServletRequest request) {
        ...
        return !Validate.isNull(userAgent) && userAgent.contains("micromessenger");
    }

    public static String getTokenType(HttpServletRequest request) {
        if (isWeChat(request)) {
            return Res.ATTEND_WAY_WX;
        } else {
            if (isMobileDevice(request)) {
                if (isIOSDevice(request)) {
                    return Res.ATTEND_WAY_PG;
                } else {
                    return Res.ATTEND_WAY_AZ;
                }
            } else {
                return Res.ATTEND_WAY_PC;
            }
        }
    }
}
